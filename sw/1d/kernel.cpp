...
// CORES_PER_GROUP controls the pipeline length (how many cores share one sequence).
// Defaults to bsg_tiles_Y (8) for backward compatibility: 16 groups of 8 cores.
// Can be set to any power of two from 1 to NUM_TILES (128).
// Tiles are numbered column-major: tile_id = x * bsg_tiles_Y + y.
// This keeps the Y pipeline intact for small groups, and naturally extends
// across X columns for groups larger than bsg_tiles_Y.
...
// Convert 1D tile ID back to physical (x, y).
...
extern "C" int kernel(uint8_t* qry, uint8_t* ref, int* output, int pod_id)
{
  ...
  // Remote pointers computed from 1D tile IDs — works for any CORES_PER_GROUP.
  ...
  mailbox_t   *next_mailbox       = (mailbox_t *)bsg_remote_ptr(
      TILE_X(next_id), TILE_Y(next_id), &mailbox);
  volatile int *prev_next_is_ready = (volatile int *)bsg_remote_ptr(
      TILE_X(prev_id), TILE_Y(prev_id), (void*)&next_is_ready);
